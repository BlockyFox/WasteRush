...
func _ready() -> void:
	var parent := get_parent() # get the first parent
	while true: # keep looping until we stop
		assert(not parent == null, "Could not find Base") # we didn't fine any StageBase in the tree
		if is_instance_of(parent, StageBase):
			var stageBase := parent as StageBase # this is just for type safety and not strictly needed
			banana.connect(stageBase._on_garbage_bag_banana) # connect the signal
			break # stop looping
		parent = parent.get_parent() # get the next node up the hierarchy
